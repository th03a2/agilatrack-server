...
const getSmtpConfig = () => {
  const emailProvider = normalizeText(process.env.EMAIL_PROVIDER || "gmail").toLowerCase();
  ...
  const configs = {
    ...
    custom: {
      host: normalizeText(process.env.SMTP_HOST) || "smtp.gmail.com",
      port: Number(process.env.SMTP_PORT || 587),
      secure: normalizeFlag(process.env.SMTP_SECURE) === "true",
    }
  };

  const config = configs[emailProvider] || configs.gmail;
